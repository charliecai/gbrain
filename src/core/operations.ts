...
import { embeddingConfigFromEnv } from './embedding.ts';
...
const put_page: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // existing try/catch around embed only catches; without a key the OpenAI-compatible
    // client would attempt 5 retries with exponential backoff (up to ~2 minutes total)
    // before giving up. Detect early using the same provider resolution as embed().
    const noEmbed = !embeddingConfigFromEnv().apiKey;
