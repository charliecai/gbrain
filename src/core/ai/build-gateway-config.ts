...
function hasProviderPrefix(model: string | undefined): boolean {
  return !!model && /^[a-z0-9][a-z0-9-]*:/i.test(model);
}
...
export function buildGatewayConfig(c: GBrainConfig): AIGatewayConfig {
  ...
  let embeddingModel = c.embedding_model;
  let embeddingDimensions = c.embedding_dimensions;
  const legacyEmbedModel = process.env.GBRAIN_EMBEDDING_MODEL || process.env.GBRAIN_EMBED_MODEL;
  const legacyEmbedDims = process.env.GBRAIN_EMBEDDING_DIMENSIONS || process.env.GBRAIN_EMBED_DIMENSIONS;
  const legacyEmbedKey =
    process.env.GBRAIN_EMBEDDING_API_KEY ||
    process.env.GBRAIN_EMBED_API_KEY ||
    process.env.SILICONFLOW_API_KEY;
  const legacyEmbedBaseUrl =
  ...
  if (legacyEmbedDims && embeddingDimensions === undefined) {
    const parsed = Number.parseInt(legacyEmbedDims, 10);
    if (Number.isFinite(parsed) && parsed > 0) embeddingDimensions = parsed;
  }
  if (legacyEmbedKey) {
    const model = legacyEmbedModel || (embeddingModel && !hasProviderPrefix(embeddingModel) ? embeddingModel : undefined) || 'text-embedding-3-large';
    if (legacyEmbedBaseUrl) {
      envFromConfig.LITELLM_API_KEY = legacyEmbedKey;
      envBaseUrls.litellm = legacyEmbedBaseUrl;
      embeddingModel = hasProviderPrefix(model) ? model : `litellm:${model}`;
    } else {
      envFromConfig.OPENAI_API_KEY = envFromConfig.OPENAI_API_KEY || legacyEmbedKey;
      embeddingModel = hasProviderPrefix(model) ? model : `openai:${model}`;
    }
    embeddingDimensions = embeddingDimensions ?? 1536;
  }

  let expansionModel = c.expansion_model;
  const legacyExpansionModel = process.env.GBRAIN_QUERY_EXPANSION_MODEL;
  const legacyExpansionKey = process.env.GBRAIN_QUERY_EXPANSION_API_KEY || process.env.GBRAIN_EXPANSION_API_KEY;
  const legacyExpansionBaseUrl = process.env.GBRAIN_QUERY_EXPANSION_BASE_URL || process.env.GBRAIN_EXPANSION_BASE_URL;
  if (legacyExpansionModel || legacyExpansionKey || legacyExpansionBaseUrl || process.env.GBRAIN_QUERY_EXPANSION_PROVIDER === 'openai_compatible') {
    const model = legacyExpansionModel || process.env.GBRAIN_EXPANSION_MODEL || 'deepseek-v4-flash';
    if (legacyExpansionKey && !process.env.DEEPSEEK_API_KEY) envFromConfig.DEEPSEEK_API_KEY = legacyExpansionKey;
    if (legacyExpansionBaseUrl) envBaseUrls.deepseek = legacyExpansionBaseUrl;
    expansionModel = hasProviderPrefix(model) ? model : `deepseek:${model}`;
  }
