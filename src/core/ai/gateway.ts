...
export function diagnoseEmbedding(modelOverride?: string): EmbeddingDiagnosis {
  ...
  // OpenAI-compatible recipes with empty model allow-lists accept arbitrary
  // user-provided model ids. `parseModelId()` already rejects empty
  // `provider:` strings; once we have a non-empty parsed.modelId, the model is
  // set and the remaining checks should decide availability.
