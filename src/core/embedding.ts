...
/**
 * Back-compat shim for pre-gateway local/OpenAI-compatible embedding config.
 *
 * Runtime embedding now routes through `src/core/ai/gateway.ts`, but forks and
 * local scripts may still inspect these env vars directly. Keep this pure
 * resolver so old SiliconFlow/local-provider setups can be translated by
 * buildGatewayConfig without resurrecting the legacy client path.
 */
export function embeddingConfigFromEnv(env: NodeJS.ProcessEnv = process.env): EmbeddingConfig {
  ...
  const parsedDimensions = rawDimensions ? Number.parseInt(rawDimensions, 10) : EMBEDDING_DIMENSIONS;
  const dimensions = Number.isFinite(parsedDimensions) && parsedDimensions > 0
    ? parsedDimensions
    : EMBEDDING_DIMENSIONS;

  return {
    model: env.GBRAIN_EMBEDDING_MODEL || env.GBRAIN_EMBED_MODEL || EMBEDDING_MODEL,
    dimensions,
